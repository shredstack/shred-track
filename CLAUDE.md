...
To run the iOS shell against your local Next.js dev server, set a single env var in `.env.local`:

```
NEXT_PUBLIC_NGROK_DOMAIN=<your-domain>.ngrok-free.dev
```

Its **presence** is the toggle; its **value** is the ngrok hostname used everywhere (Capacitor `server.url`, Next.js `allowedDevOrigins`, and the browser Supabase client's base URL). Comment it out to switch back to production mode. Because `.env.local` is gitignored, there is no per-session source file to revert before merging.

Then start the tunnel and dev server:
...
Force-quit and relaunch the iOS app from the home screen to clear the WKWebView JS cache.

See `claude_code_instructions/native_app/ios_local_dev_flag_spec.md` for the full design.

### Email confirmation gotcha

Local Supabase signups send a confirmation email to **Mailpit** (`http://127.0.0.1:54354` on the Mac), which the phone cannot reach. Options:
...
2. View the email in Mailpit on the Mac, then AirDrop or paste the confirmation link to the phone.
3. Skip signup entirely and log in with an existing local account (or seed one).

### Known limitations

These are not bugs in your setup — they are inherent constraints of the tooling:

- **Google OAuth does not work via ngrok.** The Supabase + Google Cloud OAuth clients are configured for the production redirect URL only. Use email/password or Apple Sign-In for local iOS testing.
- **Supabase Realtime (WebSocket subscriptions) does not work via ngrok.** Next.js rewrites are HTTP-only. Basic auth + REST queries are unaffected.
- **Mailpit signup confirmation emails are not reachable from the iPhone** (Mailpit listens on `127.0.0.1:54354`). See the email confirmation gotcha above.
- **You must run `npx cap sync ios` after toggling the flag.** Capacitor bakes `server.url` into the iOS bundle at sync time, not runtime.
- **You must force-quit + relaunch the iOS app after each sync.** WKWebView caches JS aggressively; a normal Cmd+R from Xcode does not always evict it.
- **You must restart `next dev` after toggling the flag.** `NEXT_PUBLIC_*` is inlined into the client bundle at build time.
- **Stale cookies between modes can cause confusing auth states.** The `sb-shredtrack` cookie is set against whichever host you last authenticated on (`localhost` vs ngrok domain). After switching modes, clear site data or use a fresh browser/device.
