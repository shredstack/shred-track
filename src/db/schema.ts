...
export const users = pgTable("users", {
  ...
  unitPreference: text("unit_preference").default("imperial").notNull(), // 'imperial' | 'metric'
  image: text("image"),
...
export const workouts = pgTable("workouts", {
  ...
  source: text("source").default("manual").notNull(), // 'manual' | 'parsed' | 'import'
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const hyroxTrainingPlans = pgTable("hyrox_training_plans", {
  ...
  planType: text("plan_type").notNull(), // 'foundation_12wk' | 'race_prep_8wk' | 'abbreviated_4wk' | 'maintenance'
