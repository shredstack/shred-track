...
async function backfillBenchmarkTemplates(
  ...
  for (const bw of bws) {
    ...
    const fingerprintInput = buildFingerprintInput({
      ...
      vestRequirement: (bw.vestRequirement as VestRequirement) ?? "none",
...
async function backfillWorkoutTemplates(
  ...
  for (const w of ws) {
    ...
    const fingerprintInput = buildFingerprintInput({
      ...
      rounds: w.rounds,
      vestRequirement: (w.vestRequirement as VestRequirement) ?? "none",
...
async function runVerifyAssertions(
  ...
  if (Number(orphanSessions.c) > 0) {
    throw new Error(
      `verify: ${orphanSessions.c} non-freeform sessions lack both a template and a body`
    );
  }
